...
if [ "$1" = 'remotecv' ]; then
    if [ "$WORKER_BACKEND" = 'pyres' ]; then
        exec remotecv -b $WORKER_BACKEND --host $REMOTECV_REDIS_HOST --port $REMOTECV_REDIS_PORT --database $REMOTECV_REDIS_DATABASE -l $REMOTECV_LOG_LEVEL -o $REMOTECV_LOADER -s $REMOTECV_STORE $TIMEOUT_PARAMETER $SENTRY_URL_PARAMETER
    fi
    if [ "$WORKER_BACKEND" = 'celery' ]; then
        exec remotecv -b $WORKER_BACKEND --host $REMOTECV_REDIS_HOST --port $REMOTECV_REDIS_PORT --database $REMOTECV_REDIS_DATABASE --region $SQS_QUEUE_REGION --key_id "$SQS_QUEUE_KEY_ID" --key_secret "$SQS_QUEUE_KEY_SECRET" -l $REMOTECV_LOG_LEVEL -o $REMOTECV_LOADER -s $REMOTECV_STORE $TIMEOUT_PARAMETER $SENTRY_URL_PARAMETER worker
    fi
fi
